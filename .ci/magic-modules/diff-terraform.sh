...
if [ "$(git rev-parse HEAD)" != "$OLD_COMMIT_SHA" ]; then
    MESSAGE="${MESSAGE}[View Diff](https://github.com/modular-magician/$PROVIDER_NAME/compare/$BRANCH_NAME-previous...$BRANCH_NAME)"
else
...
git clone "magic-modules-previous/build/$SHORT_NAME" "./terraform-diff/new"
