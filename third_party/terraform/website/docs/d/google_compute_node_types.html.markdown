...
* `zone` (Optional) - The zone to list versions for. Should be in region/zone
of node template/node group (respectively) that will include nodes of this type.
If `zone` is not specified, the provider-level zone must be set and is used
instead.
